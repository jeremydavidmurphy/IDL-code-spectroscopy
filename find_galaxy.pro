;######################################################################
;
; Copyright (C) 1999-2009, Michele Cappellari
...
;           FRACTION=fraction, INDEX=ind, LEVELS=levels, NBLOB=nblob, $
;           /PLOT, /QUIET, X=x, Y=y
;
; INPUTS:
...
;       V1.25: Included LEVELS input keyword. MC, Oxford, 3 June 2009
;       V1.26: Minor changes to the plotting. MC, Oxford, 14 October 2009
;-
;----------------------------------------------------------------------------
...
PRO find_galaxy, img, majoraxis, eps, theta, xpeak, ypeak, xmed, ymed, $
     FRACTION=fraction, PLOT=PLOT, QUIET=quiet, NBLOB=nblob, $
     INDEX=ind, X=x, Y=y, LEVEL=level
COMPILE_OPT IDL2
ON_ERROR, 2
;
...
if n_elements(level) eq 0 then level = a[j[s[4]*(1.0-fraction)]] 
j = WHERE(a GT level)
...
IF KEYWORD_SET(plot) THEN BEGIN
    ...
    xc = xmed/ratio
    yc = ymed/ratio
    tvellipse, mjr, mjr*(1-eps), xc, yc, theta-90, COLOR=200, THICK=3
    ang = [0,!pi] + theta/!radeg
    plots, xc - mjr*sin(ang), yc + mjr*cos(ang), COLOR=40, /DEVICE, THICK=3 ; Major axis    
    plots, xc + mjr*cos(ang), yc + mjr*sin(ang), COLOR=40, /DEVICE, THICK=3, LINE=2
ENDIF
