...
readcol,'listvp1',files,FORMAT='A'
num = n_elements(files)
on1='all_vp1.ps'
on2='F/#!dout!n Values for 7 VP1 Fibers'

colour = [60,110,180,150]

data = dblarr(30,8,num)
temp = dblarr(8,30)
names = strarr(num)
FOR k=0,num-1 DO BEGIN
    name = strsplit(files[k],'.',/EXTRACT)
    openr,5,files[k]
    readf,5,temp
    free_lun,5
...
FOR j=0,num-1 DO BEGIN
    ee[*,j] = data[*,0,j]
    temp = data[*,7,j]
    fnumber[*,j] = reverse(temp)
ENDFOR
xulim = max(fnumber)+0.1
xllim = min(fnumber)-0.1

;now all the data is read in
ans=''
;rint,'Name the output plot (W/O the .ps):'
;ead,on1
;n1 = on1+'.ps'
;rint,'Title the plot:'
;ead,on2
...
ulim95 = min(data[25,7,*])
llim95 = max(data[25,7,*])

ly2 = [0.88,1.02]
lx2a = [llim95, llim95]
lx2b = [ulim95, ulim95]
l1 = [xulim-0.045,xulim-0.17]
l2 = [1.03,1.03]
...
FOR k=0,qtr-1 DO slice[*,k] = data[*,0,k*4]
FOR j=0,29 DO mee[j] = mean(slice[j,*])

FOR k=0,qtr-1 DO slice[*,k] = data[*,7,k*4]
...
FOR k=0,qtr-1 DO slice[*,k] = data[*,7,k*4+1]
...
FOR k=0,qtr-1 DO slice[*,k] = data[*,7,k*4+2]
...
FOR k=0,qtr-1 DO slice[*,k] = data[*,7,k*4+3]
