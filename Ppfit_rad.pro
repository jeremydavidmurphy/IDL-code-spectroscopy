; This routine is used to plot the output of pfitlov.out the 4 moments
; of the 

; Generates to screen and postscript plots of the velocity,
; v. dispersion, h3 and h4 parameters from a given pfitlov.out
; file. It does this against radius.

; The output PS files are named "galname_v.ps" and "galname_h.ps"

; MODIFIED ON JAN 2010: The only meaningful change is in the
; configuration of the output figures. Now you select log or regular
; radial axes and the plot has all 4 of them vertical. THIS ALSO NO
; LONGER PLOTS TO SCREEN!

; MODIFIED ON APRIL 22, 2012: This code used to do a bunch of weird
; parsing of the input lists. Now the bin.radius list must be in the
; following form:
; bin 01  3.45
; bin 02  4.32
; etc.
; There the second column in the angular position and the third is the
; radius. The first column is needed to know which to place a negative
; value in front of = bin vs. bnn
...
readcol,file,format='a,f,f,f,f',bin1,vel,disp,h3,h4
n0 = n_elements(bin1)

readcol,'bin.radius',format='a,i,f',bin2,index,rad
n1 = n_elements(bin2)
