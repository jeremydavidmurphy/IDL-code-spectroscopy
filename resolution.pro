...
; This code calculates the resolution values needed to convolve with
; the template stars during the fitlov and fitlovc routines. The
; routine has two steps. The first reads in the bin and data lists,
; then calculates the instrumental resolution (with weights) for each
; bin in 4 discrete wavelength ranges (look for the ***'s to change
; these ranges).
