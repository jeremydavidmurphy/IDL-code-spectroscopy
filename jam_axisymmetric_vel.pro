;###############################################################################
;
; Copyright (C) 2008-2010, Michele Cappellari
...
;     - KAPPA can have positive and negative elements to model 
...
; V2.11: Added keyword RBH. MC, Oxford 4 April 2009
; V2.12: Do not change KAPPA on output.
;     Compute FLUX even when not plotting. MC, Oxford, 29 May 2009
; V2.13: Skip unnecessary interpolation when computing few points
;     without PSF convolution. After feedback from Eric Emsellem.
;     MC, Oxford, 6 July 2009
; V2.14: The routine TEST_JAM_AXISYMMETRIC_VEL with the usage example now
;     adopts a more realistic input kinematics. MC, Oxford, 08 February 2010
; V2.15: Forces q_lum && q_pot < 1. MC, Oxford, 01 March 2010
; V2.16: Allow for counter-rotating components as described
...
;
sum = 0d
nu = sign(kappa)*kappa^2*dens_lum*exp(-0.5d/s2_lum*(R2 + z2/q2_lum))
f = dani + 1d - bani*q2_lum
for j=0,n_elements(dens_pot)-1 do begin ; loop over mass Gaussians
    ...
    for k=0,n_elements(dens_lum)-1 do begin ; loop over luminous Gaussians
        ...
        d = f[k] - ((1d - bani[k])*c + e2_pot[j]*bani[k])*u2 ; equation (23)
        sum += nu[k]*e*d/(1d - c*u2) ; equation (38)
    endfor
endfor
...
for j=0,n-1 do begin ; loop over all z1 values along the LOS
    ...
    int[j] = k1 ? sqrt(nu*tmp>0) : sign(nu*tmp)*sqrt(abs(nu*tmp)) ; Note 8 pg. 77
...
if max(s.sigmaPsf) gt 0 and s.pixSize gt 0 then begin ; PSF convolution
    ...
    if s.NRAD*s.NANG ge nbins then begin
        ...
        for j=0,nbins-1 do begin
            wm2 = janis1_weighted_first_moment(x[j], y[j], inc_deg, s)
...
pro jam_axisymmetric_vel, $
    ...
    STEP=step, NRAD=nrad, NANG=nang, RBH=rbh
compile_opt idl2
on_error, 2

ngau = n_elements(surf_lum)
if n_elements(beta1) eq 0 then beta = sigma_lum*0   ; Anisotropy parameter beta = 1 - (sig_z/sig_R)^2 (beta=0-->circle)
...
if n_elements(kappa1) eq 0 then kappa = sigma_lum*0+1d ; Anisotropy parameter V_obs = kappa * V_iso (kappa=1-->circle)
...
if n_elements(gamma1) eq 0 then gamma = sigma_lum*0  ; Anisotropy parameter gamma = 1 - (sig_phi/sig_R)^2 (gamma=0-->circle)
...
if n_elements(evel) eq 0 and n_elements(vel) gt 0 then evel = vel*0+5 ; Constant 5km/s errors
...
if mbh gt 0 then begin
    sigmaBH_pc = rbh*pc ; Adopt for the BH just a very small size sigma=0.01"
...
if n_elements(vel) gt 0 then begin
    ...
    ;
    kappa2 = total(abs(vel[goodBins]*xbin[goodBins])) $
           / total(abs(velModel[goodBins]*xbin[goodBins]))

    if n_elements(kappa1) gt 0 then kappa1 *= kappa2 else kappa1 = kappa2 ; output anisotropy
    ...
    ;
    kappa3 = total(vel[goodBins]*velModel[goodBins]/evel[goodBins]^2) $
           / total((velModel[goodBins]/evel[goodBins])^2)

    velModel *= kappa2*sign(kappa3)

    chi2 = total( ((vel[goodBins]-velModel[goodBins])/evel[goodBins])^2 ) / n_elements(goodBins)
    if ~keyword_set(quiet) then print, inc, chi2, kappa2, beta[0], mbh, $
        FORMAT='("inc: ", f0.1, "; chi^2/DOF: ", g0.3, "; kappa: ", f0.2, "; beta: ", f0.2, "; M_BH: ", e0.1)'

    if keyword_set(plot) then begin
        str = 'i=' + STRING(inc,FORMAT='(f0.1)') + $
              ' !7b!X!Dz!N=' + STRING(beta[0],FORMAT='(f0.2)') + $
              ' !7j!X=' + STRING(abs(kappa2),FORMAT='(f0.2)') + $
...
;
ml = -1 ; Request the M/L to be fitted
beta = 0.3 ; Adopted anisotropy
jam_axisymmetric_rms, $
    surf_lum, sigma_lum, qobs_lum, surf_pot, sigma_pot, qobs_pot, $
    inc, mbh, distance, xbin, ybin, rmsModel, ML=ml, $
    BETA=beta, RMS=rms, /PLOT, SIGMAPSF=0.6, PIXSIZE=0.8

; The velocity is fitted at the best fitting M/L, beta_z and
; inclination determined at the previous stage
;
surf_pot = surf_pot*ml ; Scale the density by the best fitting M/L
jam_axisymmetric_vel, $
    surf_lum, sigma_lum, qobs_lum, surf_pot, sigma_pot, qobs_pot, $
    inc, mbh, distance, xbin, ybin, velModel, $
    BETA=beta, VEL=vel, /PLOT, SIGMAPSF=0.6, PIXSIZE=0.8, NRAD=20
