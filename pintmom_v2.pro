...
x1 = 0.06 ;the ps plotting ranges
x2 = 1000
xs = [0.06,450] ;the range over which to plot the stars
xg = [200,1100] ;the range over which to plot the GCs
y1 = 0.4
y2 = 1.6
...
if (n00 eq 1) then begin
    cd,directory
    readcol,'intmomSTARS',format='f,f,f,f,x,f,f,f',R,theta,sigR,St,Sp,vp,beta,skipline=1,silent=1
    n0 = n_elements(R)
    n1 = n_elements(angles)
    n2 = n0 / n1
    sigarrS = dblarr(n2,2,n1)
    radiusS = fltarr(n2,2)
...
;and the same thing for the GCs
    readcol,'intmomGC',format='f,f,f,f,x,f,f,f',R,theta,sigR,St,Sp,vp,beta,skipline=1,silent=1
    n0 = n_elements(R)
    n1 = n_elements(angles)
    n2 = n0 / n1
    ...
    for j=0,n0-1 do begin
        ...
        if (t1 eq t2) then begin
            ...
            cntr2 = cntr2 + 1
            if (cntr1 eq 0) then radiusR[j,0] = R[j]
        endif else begin
            cntr2 = 0
            cntr1 = cntr1 + 1
            sigarrG[cntr2,0,cntr1] = sigR[j]
...
if (n00 gt 1) then begin
    for jj=0,n00-1 do begin
        cd,directory[jj]
        readcol,'intmomSTARS',format='f,f,f,f,x,f,f,f',R,theta,sigR,St,Sp,vp,beta,skipline=1,silent=1
        readcol,'intmomGC',format='f,f,f,f,x,f,f,f',R,theta,sigR2,St2,Sp2,vp2,beta2,skipline=1,silent=1
        if (jj eq 0) then begin
            ...
            sigarrS = dblarr(n2,2,n1)
            sigarrG = dblarr(n2,2,n1)
            radiusS = fltarr(n2,2,n1)
            radiusG = fltarr(n2,2,n1)
        endif
...
;'xg') are driven out of the plotting range by being set to -1000
    ...
    if (scale gt 0) then begin
        plot,radS,stars,/nodata,/xlog,xrange=[x1,x2],xstyle=9,xtitle='Radius (arcsec)',$
          ytitle='!7r!3!dr!n/!7r!3!dt',yrange=[y1,y2],/ystyle,xthick=4,ythick=4,$
          charthick=3,charsize=1.2,position=[0.13,0.13,0.93,0.89]
    endif
    if (scale eq 0) then begin
        plot,radS,stars,/nodata,/xlog,xrange=[x1,x2],xstyle=1,xtitle='!6Radius (arcsec)',$
          ytitle='!7r!3!dr!n/!7r!3!dt',yrange=[y1,y2],/ystyle,xthick=4,ythick=4,$
          charthick=3,charsize=1.2,position=[0.13,0.13,0.93,0.89],title=galaxy
    endif
    ...
    if (scale ne 0) then begin
        loadct,0
        axis,xaxis=1,xtitle='!6R (kpc)',xrange=[radkpc1,radkpc2],/save,$
          charthick=3,charsize=1.2,xstyle=1,xthick=4
    endif
