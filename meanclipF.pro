FUNCTION meanclipF, Data, Mean, Sigma, CLIPSIG=clipsig, MAXITER=maxiter, $
    CONVERGE_NUM=converge_num, VERBOSE=verbose, SUBS=subs
;+
; NAME:
;	MEANCLIPF (I totally stole this code...)
;
; PURPOSE:
...
IF n_params(0) LT 1 THEN BEGIN
    print, 'CALLING SEQUENCE:  MEANCLIP, Data, Mean, Sigma'
...
subs = where(finite(data),ct)
iter=0
REPEAT BEGIN
    skpix = data[subs]
    iter = iter + 1
    lastct = ct
    medval = median(skpix,/even) ;JDM
    sig = stddev(skpix,/nan)    ;JDM
    wsm = where(abs(skpix-medval) LT clipsig*sig,ct)
;    print,'The number of tossed data points at iteration #'+strn(iter) ;JDM
;    print,'is: '+strn(n_elements(data)-ct)+'...' ;JDM
    IF ct GT 0 THEN subs = subs[wsm]
ENDREP UNTIL (float(abs(ct-lastct))/lastct LT converge_num) $
          OR (iter GT maxiter)

mom = moment(data[subs])
mean = mom[0]
sigma = sqrt(mom[1])
