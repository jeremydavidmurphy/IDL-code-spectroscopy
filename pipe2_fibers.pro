...
;*****************************************************************
pro pipe2_fibers, LIST=listoflists, BIN=yourbinlist,$
...
;IFU = 'vp1'
IFU = 'vp2'
if (IFU eq'vp2') then tfibers = 246.0
if (IFU eq'vp1') then tfibers = 247.0
...
if (IR eq 'yes') and (COMP eq 'areto') then begin
    if focalreducer eq 'old' and IFU eq 'vp1' then $
...
;-------------------------------------------------------------------------------
        ...
        outnamesnA1 = binname+'_'+onelisttrim[0]+'_S2Nalt1.fits'
        outnamesnA2 = binname+'_'+onelisttrim[0]+'_S2Nalt2.fits'
...
; (n_fibers/total_fiber#)^2 allsky1_i + (same sum for allsky2)
        ...
        S2N = fltarr(n1)
;        skyfactor = (tsci/(2.0*tsky))^2 * (nfibers/tfibers)^2
;        skyfactor = 1.0
;        skyfactor = (120./246.)^2
        skyfactor = (1.0/(246.0 * 3.0))^2
        for s=0,n1-1 do begin ;a loop over wavelength
            signal = total(alldata[s,*])
            noise2 = total(allnoise2[s,*]) + skyfactor * (total(allsky1[s,*]) + total(allsky2[s,*]))
            noise = sqrt(noise2)
            S2N[s] = signal / noise
            array[s,1] = noise
         endfor
        writefits,outnamesn,S2N,s2nhead
; The above S/N calculation is essentially the classic formula. 
...
;------------------------------------------------------------------------------
            if (j eq 0) then sxaddpar,datahead,'COMMENT','  The spectra was made via a straight summation'
            temp1 = alldata
...
;            alldata = 0.0
        ...
        array[*,0] = outdata
        array[*,2] = wave
        tempname = binname+'.fits'
        writefits,tempname,array,datahead
;------------------------------------------------------------------------------
