...
lastplot = 'off' ;This just plots the final spectra for each bin once complete.
...
swch1 = 'on' & swch2=swch1 & swch3=swch1 & swch4=swch1 & swch5=swch1 & swch6=swch1 & swch7=swch1 & swch8=swch1 & swch9=swch1
...
;            tfibers = uint(tfibers) ;***
            ...
            if (count3 gt 0) then begin
               for k=0,count3-1 do begin
                  ...
                  noise[*,y[bfi[k]]-step:y[bfi[k]]+step] = -666.0       ;bad fibers are flagged as -666 flags to go through realignF. they are flagged as = 0.0 later
...
;                if (TRANS eq 'yes') then sxaddpar,headout,'COMMENT','  This spectra has been corrected for airmass transparency'
            ...
            i666 = where(dataA eq -666)
            if (i666[0] ne -1) then noiseA[i666] = -666 ;the bad pixels are flagged in the noise array as well. I do this because I am rejecting the pixels, so I should also be rejecting the noise.
            ...
            col = median(skynoise1A,dim=2) ;just a single median-collapsed spectra is written out for the sky noise.
            writefits,skyfilenames[0,j],col,weighthead ;***
            col = median(skynoise2A,dim=2)
...
;-------------------------------------------------------------------------------
    ...
    for j=0,n0-1 do begin      ;a loop though each bin in the bin.list
            ...
            if (countP gt 0) and (countB gt 0) then begin ;if both the pointing exists AND the point is used
                for k=0,n_elements(ipp)-1 do begin ;a loop over each exposure for a given pointing
                    ...
                    if (k eq 0) then begin
                        ...
                        skynoise1 = sky1 * float(n_elements(imma)) * skyfactor;the multiplication here accounts for the number of rows per fiber and number of fibers coming from a given frame.
                        skynoise2 = sky2 * float(n_elements(imma)) * skyfactor
                        ...
                        skynoise1 = [[skynoise1],[sky1 * float(n_elements(imma)) * skyfactor]]
                        skynoise2 = [[skynoise2],[sky2 * float(n_elements(imma)) * skyfactor]]
        ...
        allfibnorm = median(alldata,dim=1) ;array for the fiber normalization
        ineg = where(allfibnorm le 0.0 and allfibnorm gt -660.0,countneg) ;04/22/2011: To catch the possibility of a negative fiber normalization (due to oversubtraction of the sky, most likely)
        if (countneg gt 0) then begin
           allfibnorm[ineg] = -666
           alldata[*,ineg] = -666 ;04/22/2011: Any oversubtracted fiber row is set to -666 and later rejected
           allnoise[*,ineg] = -666
        endif

        index = where(allfibnorm ne -666,count) ;an index of the GOOD rows
...
;------------------------------------------------------------------------------
        ...
        if (IR eq 'yes') then begin ; The instrumental response is backed out of the data (added on 12/22/10)
           ...
           for k=0,nALL-1 do begin
              if allfibnorm[k] eq -666 then goto, stepoverbadfiber
...
;        if (countbad gt 0) then alldataCOR[i666] = 0.0 ;the signal is set to zero so as not to mess with the S2N calculation.
;        if (countbad gt 0) then allnoiseCOR[i666] = 0.0 ;the noise is set to zero so as not to be included in the noise calculation.

        nfibers = n_elements(alldata[0,*])/5.0 ;***
        nfibers = nfibers/14.0
        S2N1  = fltarr(n1)
        S2N2  = fltarr(n1)
        S2N1C = fltarr(n1)
        S2N2C = fltarr(n1)

        skyfactor = (nfibers/tfibers)^2 * ((2*tsky)/tsci)^2

        for s=0,n1-1 do begin ;a loop over wavelength
            ; the classical S/N method (sum_i signal_i / sqrt(sum_i noise_i^2))



            sig1 = total(alldata[s,*])
            noi1 = sqrt(total(allnoise[s,*]^2) + (skyfactor * 5 * nfibers * (sky1[s]^2 + sky2[s]^2)))
            S2N1[s] = sig1/noi1
 



           ; the method from Cappellari '03, based on Robertson '86 (THIS METHOD WORKS ONLY FOR DATA THAT'S BEEN WEIGHTED W_i = S_i/N_i^2)
            temp = fltarr(n_elements(alldata[s,*]))
            for ss=0,n_elements(alldata[s,*])-1 do begin
               sig1 = alldata[s,ss]^2
               noi1 = allnoise[s,ss]^2 + (skyfactor * 5 * nfibers * (sky1[s]^2 + sky2[s]^2))
               temp[ss] = sig1/noi1
            endfor
            S2N2[s] = sqrt(total(temp))
            ; the corrected versions of both...
            sig1 = total(alldataCOR[s,*])
            noi1 = sqrt(total(allnoiseCOR[s,*]^2) + (skyfactor * 5 * nfibers * (sky1COR[s]^2 + sky2COR[s]^2)))
            S2N1C[s] = sig1/noi1
            temp = fltarr(n_elements(alldata[s,*]))
            for ss=0,n_elements(alldata[s,*])-1 do begin
               sig1 = alldataCOR[s,ss]^2
               noi1 = allnoiseCOR[s,ss]^2 + (skyfactor * 5 * nfibers * (sky1COR[s]^2 + sky2COR[s]^2))
               temp[ss] = sig1/noi1
            endfor
            S2N2C[s] = sqrt(total(temp))
         endfor
...
; satisfying manner.
        ...
        alldataS = alldata
        alldataSCOR = alldataCOR

        for k=0,nFINAL-1 do alldataS[*,k] = alldata[*,k] / allfibnorm[k] ;the data is scaled to be at approximately the same flux.
        for k=0,nFINAL-1 do alldataSCOR[*,k] = alldataCOR[*,k] / allfibnorm[k] ;the data is scaled to be at approximately the same flux.

        if (countbad ne 0) then alldataS[i666] = -666.0 ;flags are preserved so as to be handled properly in the biweight (or other combination)        
        if (countbad ne 0) then alldataSCOR[i666] = -666.0 ;flags are preserved so as to be handled properly in the biweight (or other combination)
...
;-------------------------------------------------------------------------------
        ...
        ;the bad rows are tossed, BUT THERE ARE STILL BAD PIXELS IN THES DATA!!!
        alldata = alldata[*,index] ;for large arrays, this step is a killer. 
        allnoise = allnoise[*,index]
...
;        for kk=0,7 ,2 do begin
;           outdata = float(array[*,kk])
...
;        s2narray[*,wgtcntr,0] = outdata
;        endfor
;           wgtcntr = wgtcntr + 1
           writefits,outnamed,array,datahead
...
jumpendend:
;writefits,'noise2.fits',s2narray,s2nhead
