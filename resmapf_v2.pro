FUNCTION resmapf_v2, wave, IRarray, smth, med, nameR, nameS, month_day
...
; MODIFIED ON MAY 15, 2011: Some changes to what's fed into this code
; and what's written out have been made. The older version is now
; saved as resmapF_v1.pro. The text files are no longer written out.

; MODIFIED ON SEPT 7, 2011: This is a simplified version of
; resmapF. It's cut out the smoothing and writing out the text
; versions. This routine is now essentially a plotting routine and
; nothing more.
...
;temp = [[sfwhm],[wave]]
;writefits,nameS+'.fits',temp ;the smoothed version is written out as a fits file
;temp = readfits(nameS+'.fits',header,/silent)
;sxaddpar,header,'COMMENT',' A smoothed version of the IR map.'
...
psname = 'IRmap_'+month_day+'.ps'
set_plot,'ps'
device,file=psname
loadct,0,/silent
shade_surf,sfwhm,Pwave,Py,charsize=2.0,title='Smoothed FWHM values',$
  zrange=[zdn,zup],/zstyle,ytitle='Fiber',ztitle='FWHM (A)',xthick=2,ythick=2,$
  zthick=2,charthick=2
device,/close
set_plot,'x'
