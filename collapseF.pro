...
; way to approach this in the future would be to take the median, or
; replace the missing values with the median of the others.
...
for jj=0,n2-1 do begin ;a loop through each fiber
    index = uint(indexarr[jj])
    rowD = data[*,index[jj]-step:index[jj]+step]
    rowW = weight[*,index[jj]-step:index[jj]+step]
    print,rowD[1000,*]
    print,rowW[1000,*]
    pause
